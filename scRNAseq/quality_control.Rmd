...
update.packages(ask = FALSE)  # Automatically choose to update all out-of-date packages
...
# Check the assays present in each Seurat object
sapply(split_seurat, function(x) names(x@assays))

# Normalize and scale the data for each Seurat object
...
# Perform cell cycle scoring for each Seurat object
split_seurat <- lapply(split_seurat, function(x) {
  x <- CellCycleScoring(x, s.features = s_genes, g2m.features = g2m_genes)
  return(x)
})

# Perform SCTransform for each Seurat object
split_seurat <- lapply(split_seurat, function(x) {
  x <- SCTransform(x, vars.to.regress = c("mitoRatio", "S.Score", "G2M.Score"))
  return(x)
})

# Check the assays present in each Seurat object after SCTransform
sapply(split_seurat, function(x) names(x@assays))

# Select integration features
integ_features <- SelectIntegrationFeatures(object.list = split_seurat, nfeatures = 3000)

# Prepare the Seurat objects for integration
split_seurat <- PrepSCTIntegration(object.list = split_seurat, anchor.features = integ_features)

# Check the assays present in each Seurat object after PrepSCTIntegration
sapply(split_seurat, function(x) names(x@assays))
...
# Run PCA and UMAP on the integrated data
seurat_integrated <- RunPCA(object = seurat_integrated)
seurat_integrated <- RunUMAP(object = seurat_integrated, dims = 1:40)

# Visualize UMAP
DimPlot(seurat_integrated, group.by = "orig.ident", pt.size = 0.5) + NoLegend()
